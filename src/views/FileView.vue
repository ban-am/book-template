...
const getOrAddContentInfo = () => {
  ...
  contentInfo = fileProvider.getOrAddContentInfo(id, dirveProvider);
}

const render = () => {
  fileProvider.renderPage(route.params.id as string, iframeRef);
}
...
<template>
  <div class="h-full flex content-width">
    <div class="flex flex-col py-2 grow gap-2">
      <div class="p-2 bg-white rounded-sm shadow-sm flex items-center">
        <span class="font-bold">{{ contentInfo.fileInfo.name }}</span>
        <div class="grow"></div>
        <button type="button" class="k-button k-button-md k-rounded-md k-button-link with-underline" @click="onClose" >
          <span class="k-button-text">Close</span>
        </button>
      </div>

      <div class="h-full bg-white rounded-sm shadow-sm">
        <div v-if="contentInfo.loading" class="middle bg-gray-300">
          <div >LOADING</div>
        </div>
        <iframe src="javascript:void(0);" :ref="onRef" style="width: 100%; border: none; height: 100%;"></iframe>
      </div>

      <div class="p-2 bg-white rounded-sm shadow-sm flex items-center justify-between">
...
<style>
.content-width {
  --content-width: calc(100vw - 2rem - var(--vp-sidebar-width-small));
